...
Vision::Vision() {
  ...
  static_cast<WarpCorrection *>(this->_correction)
      ->initFromFile(FIELDLIMITSPATH, &this->_convert);
  static_cast<LUTSegmentation *>(this->_segmentation)
      ->initFromFile(SEGMENTATION_DEFAULT_FILE);
  ...
  this->_lastPositions =
      std::vector<std::queue<std::pair<cv::Point2d, QTime>>>(7);
  ...
  this->server = new VisionServer("224.5.23.2", 10006);
...
void Vision::update() {
  ...
  if (this->_isProcessingEnabled) {
    ...
    std::vector<std::vector<Run>> runs =
        this->_compression->run(this->_processingFrame);
    saveFrameDimensions(this->_processingFrame);
    this->_detection->run(runs, this->_processingFrame.rows,
                          this->_processingFrame.cols);
  }
}
...
void Vision::update(cv::Mat &frame, QTime timeStamp) {
  ...
  this->update();
  uint32_t actualTime =
      static_cast<uint32_t>(this->firstTime.msecsTo(timeStamp));
  // printf("%u\n", actualTime);
  ...
  if (this->_deepLogRecord) {
    ...
    if (this->_firstFrameDeepLog) {
      if (!this->_deepLogRecordingVideo) {
        ...
        this->_deepLogFileFolder =
            "Log/deep/" + date.toString("yyyy-MM-dd/").toStdString();
        this->_deepLogFileName = "log";
        this->_deepLogFilePath =
            this->_deepLogFileFolder + this->_deepLogFileName;
      }
      std::cout << "Creating Log folder " << this->_deepLogFileFolder
                << std::endl;
      ...
      std::stringstream ss;
      uint ini = actualTime;
      ss << this->_deepLogFilePath << "_" << timeStamp.hour() << "."
         << timeStamp.minute() << "." << timeStamp.second() << ".csv";
      ...
      if (this->_deepLogFile) {
        ...
        fprintf(this->_deepLogFile,
                "ID_VIDEO;TIMESTAMP_INITIAL;ID_ROBO1;ID_ROBO2;ID_ROBO3\n");
        fprintf(this->_deepLogFile, "%s;%d;%d;%d;%d\n",
                this->_deepLogFileName.c_str(), this->_deepLogInitialTime,
                entities[ROBOT1].id(), entities[ROBOT2].id(),
                entities[ROBOT3].id());
        fprintf(this->_deepLogFile,
                "R1X;R1Y;R1T;R2X;R2Y;R2T;R3X;R3Y;R3T;A1X;A1Y;A1T;A2X;A2Y;A2T;"
                "A3X;A3Y;A3T;BX;BY;TIMESTAMP_FRAME\n");
      }
    }

    if (this->_deepLogFile) {
      // any other frame
      fprintf(this->_deepLogFile, "%lf;%lf;%lf;", entities[ROBOT1].position().x,
              entities[ROBOT1].position().y, entities[ROBOT1].angle());
      fprintf(this->_deepLogFile, "%lf;%lf;%lf;", entities[ROBOT2].position().x,
              entities[ROBOT2].position().y, entities[ROBOT2].angle());
      fprintf(this->_deepLogFile, "%lf;%lf;%lf;", entities[ROBOT3].position().x,
              entities[ROBOT3].position().y, entities[ROBOT3].angle());

      fprintf(this->_deepLogFile, "%lf;%lf;%lf;", entities[ADV1].position().x,
              entities[ADV1].position().y, entities[ADV1].angle());
      fprintf(this->_deepLogFile, "%lf;%lf;%lf;", entities[ADV2].position().x,
              entities[ADV2].position().y, entities[ADV2].angle());
      fprintf(this->_deepLogFile, "%lf;%lf;%lf;", entities[ADV3].position().x,
              entities[ADV3].position().y, entities[ADV3].angle());

      fprintf(this->_deepLogFile, "%lf;%lf;%lf", entities[BALL].position().x,
              entities[BALL].position().y, entities[BALL].angle());
  ...
  this->_visionRunTime =
      static_cast<double>(this->_visionFrameTimer.getMilliseconds() * 0.2 +
...
void Vision::setQuantizationBool(bool quantization) {
  ...
  if (this->_segmentation)
    static_cast<LUTSegmentation *>(this->_segmentation)
        ->setQuantizationBool(quantization);
  this->_visionStatusLocker.unlock();
}
...
bool Vision::getQuantizationBool() {
  ...
  bool ret = false;
  if (this->_segmentation)
    ret = static_cast<LUTSegmentation *>(this->_segmentation)
              ->getQuantizationBool();
  this->_visionStatusLocker.unlock();
  return ret;
}
...
void Vision::setDeepLogFileName(std::string fileName) {
  ...
  this->_deepLogFileName = fileName.substr(std::max(
      static_cast<uint>(0), static_cast<uint>(fileName.rfind('/') + 1)));
  this->_visionStatusLocker.unlock();
}
